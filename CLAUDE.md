...
golden test. A chunked copy-on-write variant (`mvcc_*`, all three languages) snapshots via an O(#chunks) root capture at an op boundary without stopping the writer; `ultima_*` (Rust) runs the same workload through ultima_db (MVCC persistent B-tree, SMR pattern: SingleWriter + explicit versions, pinned git dep); `live_{stw,mvcc,ultima}` measure writer-observed latency while a snapshot is in flight (`writer_max` = stall). All variants emit byte-identical snapshot images, verified against the shared golden file. The grid also measures a cancel-heavy `churn` workload — `churn`, `mvcc_churn`, `live_stw_churn`, `live_mvcc_churn` (all three languages) plus `ultima_churn`, `ultima_batch_churn`, `live_ultima_churn` (Rust-only) — at a configurable order-to-trade ratio (`SMRC_OTR_BPS`, default 100 bps = 1%); cancel recycles freed order slots through an intrusive free list, captured in the snapshot as `freeHead` (schema v2), and the churn snapshot images are golden-verified byte-identical across all three languages. `rpc-roundtrip` is implemented for `sbe_udp` (Rust, UDP + zero-copy SBE), `grpc` (Go, gRPC),
