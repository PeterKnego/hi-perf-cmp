...
pub enum Mode {
    /// In-process echo server + client on an ephemeral loopback port (default).
    Loopback,
    /// Long-lived TCP + UDP echo responders bound to `0.0.0.0` (no stdout).
    Server,
    /// Connect to a remote responder, measure, emit the six result lines.
    Client,
}
...
/// Parse a TCP/UDP port env var, falling back to `default` when unset.
