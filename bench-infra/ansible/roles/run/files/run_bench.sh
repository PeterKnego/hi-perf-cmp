#!/usr/bin/env bash
# run_bench.sh <language> <focus_area> <mode>
#
...
#   GOROOT            Go install root (optional; PATH may already cover it)
#
# fs-write / thread-handoff ignore the RTT_* vars.
set -euo pipefail

usage() {
  echo "usage: $0 <rust|go|java> <network-rtt|filesystem-write|thread-handoff> <loopback|server|client>" >&2
  exit 2
}
...
case "$LANGUAGE" in
  rust)
    exec "${SRC_DIR}/rust/target/release/${FOCUS_AREA}"
    ;;
  go)
    exec "${SRC_DIR}/go/bin/${FOCUS_AREA}"
    ;;
  java)
    ...
    exec ./gradlew ":${FOCUS_AREA}:run" -q --no-daemon 2>/dev/null
    ;;
  *)
