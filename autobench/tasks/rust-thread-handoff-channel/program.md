...
**Minimize round-trip handoff latency** of the rust `thread-handoff`/`channel` cell
— a timer ping-pongs a token with a parked responder via a blocking-queue rendezvous, measured by the
...
- Mutable: the cell's own source only (rust `thread-handoff-channel`).
