...
Task overlay for the Java TCP RTT cell. Read this overlay, then run the loop
per `autobench/program.md`.

## Objective

**Minimize TCP round-trip latency** for the Java `network-rtt`/`tcp` cell — the
synchronous request/response RTT measured over `127.0.0.1` by the existing
artifact (`java/network-rtt-tcp`), which emits the result-contract lines
`rtt_p50` / `rtt_p99` / `rtt_mean` (`experiment="tcp"`, `language="java"`, unit
`ns`).
...
- Every other benchmark cell (`java/network-rtt-udp`, `.../network-rtt-quic`,
  `java/filesystem-write`, `java/thread-handoff`, and all of `rust/`, `go/`).
- `autobench/**` — the harness itself, including `run-iter`.
- All docs and specs.
- The cell's `build.gradle` dependency list — **never add a dependency** to the
  cell's build file.
...
1. **build** — `./gradlew --quiet :network-rtt-tcp:installDist` (in `java/`).
...
4. **Gate A (tests)** — `./gradlew --quiet :network-rtt-tcp:test :common:test`
   (in `java/`), so an optimization can't pass by breaking shared code.

## TSV schema

`autobench/tasks/java-network-rtt-tcp/results.tsv` (tab-separated):
