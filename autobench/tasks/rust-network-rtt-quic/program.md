...
Task overlay for the Rust QUIC RTT cell. Read this overlay, then run the loop
per `autobench/program.md`.

## Objective

**Minimize QUIC round-trip latency** for the Rust `network-rtt`/`quic` cell —
the synchronous request/response RTT measured over `127.0.0.1` by the existing
artifact (`rust/network-rtt/quic`), which emits the result-contract lines
`rtt_p50` / `rtt_p99` / `rtt_mean` (`experiment="quic"`, `language="rust"`,
...
- Every other benchmark cell (`rust/network-rtt/tcp`, `.../udp`,
  `rust/filesystem-write`, `rust/thread-handoff`, and all of `go/`, `java/`).
- `autobench/**` — the harness itself, including `run-iter`.
- All docs and specs.
- The cell's `Cargo.toml` dependency list — **never add or bump a dependency**;
  the quinn/rustls/rcgen/tokio stack is fixed.
...
**The transport I/O is owned by the QUIC library (quinn), so the raw-socket
...
- **Transport/connection config** — initial RTT hint, idle timeout, flow-control
  window sizes, stream limits, `max_idle_timeout`, `keep_alive_interval`.
...
**Never add a new dependency.** The quinn/rustls/rcgen/tokio stack is fixed —
do not bump or add libraries.
...
QUIC RTT is noisy over loopback — more so than TCP or UDP because the async
runtime and TLS record processing add jitter. **Always use the harness's
...
1. **build** — `cargo build --release -p network-rtt-quic` (in `rust/`).
...
`autobench/tasks/rust-network-rtt-quic/results.tsv` (tab-separated):
