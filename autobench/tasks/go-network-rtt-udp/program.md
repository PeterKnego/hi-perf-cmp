...
Task overlay for the Go UDP RTT cell. Read this overlay, then run the loop
per `autobench/program.md`.

## Objective

**Minimize UDP round-trip latency** for the Go `network-rtt`/`udp` cell — the
synchronous request/response RTT measured over `127.0.0.1` by the existing
artifact (`go/cmd/network-rtt-udp`), which emits the result-contract lines
`rtt_p50` / `rtt_p99` / `rtt_mean` (`experiment="udp"`, `language="go"`, unit
`ns`).
...
- Every other benchmark cell (`go/cmd/network-rtt-tcp`, `.../network-rtt-quic`,
...
- The Go module dependency list — **never add a Go module dependency**; the cell
  is intentionally std-library only (`syscall` is std and allowed).
...
1. **build** — `go build -o bin/network-rtt-udp ./cmd/network-rtt-udp` (in
   `go/`).
...
`autobench/tasks/go-network-rtt-udp/results.tsv` (tab-separated):
