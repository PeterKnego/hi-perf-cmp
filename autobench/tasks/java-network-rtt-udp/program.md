...
Task overlay for the Java UDP RTT cell. Read this overlay, then run the loop
per `autobench/program.md`.

## Objective

**Minimize UDP round-trip latency** for the Java `network-rtt`/`udp` cell — the
synchronous request/response RTT measured over `127.0.0.1` by the existing
artifact (`java/network-rtt-udp`), which emits the result-contract lines
`rtt_p50` / `rtt_p99` / `rtt_mean` (`experiment="udp"`, `language="java"`, unit
`ns`).
...
- Every other benchmark cell (`java/network-rtt-tcp`, `.../network-rtt-quic`,
...
1. **build** — `./gradlew --quiet :network-rtt-udp:installDist` (in `java/`).
...
4. **Gate A (tests)** — `./gradlew --quiet :network-rtt-udp:test :common:test`
   (in `java/`), so an optimization can't pass by breaking shared code.

## TSV schema

`autobench/tasks/java-network-rtt-udp/results.tsv` (tab-separated):
